...
int main(){
    ...
    assert(d2.setHullPresence(HULLPROTECTIONON));

    assert(d.setVideoAutorecord(false));
    assert(d2.setVideoAutorecord(false));
    //assert(d.stopStreaming());

    Eigen::initParallel();
    assert(d.useFullNavdata());
    assert(d2.useFullNavdata());
    sleep(1);
    assert(d.isUsingFullNavdata());
    assert(d2.isUsingFullNavdata());
